...
public class WidgetViews
{
	public static void setText(Context context, RemoteViews views, Currency currency, String text, boolean color, String label, int widgetId)
	{
		...
		if(color)
		{
			hide(views, R.id.vertcoinImageBW);
			show(views, R.id.vertcoinImage);
		}
		else
		{
			hide(views, R.id.vertcoinImage);
			show(views, R.id.vertcoinImageBW);
		}
		...
		if(showLabel)
		{
			...
			show(views, R.id.provider, R.id.imageLayout);
		}
		else
		{
			show(views, R.id.space);
